...
def main():
    # 单个H5文件路径
    h5_file_path = 'E:\多模态数据融合\数据集\mosi_data.h5'
    ...
    print(f"数据集总长度: {dataset_size}")

    # 方法一：动态计算划分比例
    train_size = int(0.7 * dataset_size)
    valid_size = int(0.1 * dataset_size)
    test_size = dataset_size - train_size - valid_size
    print(f"划分尺寸: 训练集={train_size}, 验证集={valid_size}, 测试集={test_size}")
    print(f"总划分大小: {train_size + valid_size + test_size}")
